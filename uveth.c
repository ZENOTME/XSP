			...
			struct netlink_ext_ack *extack)
{
	...
	printk("create uveth: %s: link sucessfully\n", dev->name);
...
{
    int res = rtnl_link_register(&veth_link_ops);
    if(res==0){
        printk("Uveth register successfully.\n");
    } else {
        printk("Uveth register fail: %d\n",res);
    }
    return 0;
}
